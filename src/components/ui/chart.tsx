    ...
    <style
      dangerouslySetInnerHTML={{
        __html: [
          ...
          colorConfig
            .map(([key, itemConfig]) => {
              const color = itemConfig.theme?.[THEMES.dark] || itemConfig.color
              return color ? `  --color-${key}: ${color};` : null
            })
            .filter(Boolean)
            .join("\n"),
...
type ChartTooltipContentProps<T = any, N = any> = {
  active?: boolean
  payload?: any[]
  className?: string
...
>(
  (
    ...
    const nestingLevel = payload.filter((item) => item.type === "none").length
...
>(({ className, hideIcon, payload, verticalAlign = "bottom" }, ref) => {
  const { config } = useChart()
...
function getPayloadConfigFromPayload(
  ...
  if (
    ...
    (typeof payloadPayload[key] === "string" || typeof payloadPayload[key] === "number")
  ) {
